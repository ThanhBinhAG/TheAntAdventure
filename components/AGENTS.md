...
## Contents
- Root: `Sidebar`, `Topbar`, `CRMShell`, `StoreProvider` (passthrough), `ToastHost`, `ConfirmHost`, `EmptyState` (shared empty lists; `access` variant for PermissionGate), `FormDraftsBar` (Clients/Agents local draft chips), `PermissionGate`, `AiCopilot*`, `PaginationBar`, `SearchableSelect`, `TravelMonthPicker` (Ant Design date picker + local registry; stores ISO date), …
...
- `settings/` — CRM Settings catalogs (countries, nationalities, …)
