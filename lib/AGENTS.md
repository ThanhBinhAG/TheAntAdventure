...
- Domains: `customers/`, `agents/`, `sales/`, `bookings/`, `feedback/`, `finance/`, `tax/`, `tour-design/`, `proposals/`, `outline/`, `pricing/`, `products/`, …
...
- `toast.ts` / `confirm.ts` / `confirm-discard.ts` are UI chrome only — hosts in `components/ToastHost` and `components/ConfirmHost`; modals use `hooks/useConfirmClose`.
