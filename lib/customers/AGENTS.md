...
- `customer-list-input.ts` — Zod list/create/patch/email-check/inquiry/comms contracts for BFF (includes optional revenue/cost/profit strings)
- `customer-repository.ts` — `server-only` PostgREST list/CRUD + profile context + inquiry/comms writes + `findDuplicateCustomerEmail` (`getServerSupabaseClient`)
