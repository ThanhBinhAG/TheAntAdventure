...
- `sync-config.ts` — `PAGE_BOOT_TABLES`, `SIDEBAR_BADGE_TABLES`, `PROFILE_LAZY_TABLES` (empty/deprecated), `SHELL_HYDRATE_TABLES` (non–Dev-B only), push waves, store key map
...
- `bff-managed-tables.ts` — blocks Customers, Agents, Sales, Gallery, Product, Pricing, Planner, Attractions, and Tour Design tables from browser snapshot pushes
