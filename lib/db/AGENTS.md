...
- `sync-config.ts` — `PAGE_BOOT_TABLES`, `SIDEBAR_BADGE_TABLES`, `PROFILE_LAZY_TABLES` (empty/deprecated), `SHELL_HYDRATE_TABLES` (empty — all CRM tables BFF-managed), push waves, store key map
...
- `bff-managed-tables.ts` — blocks Customers, Agents, Sales, Gallery, Product, Pricing, Planner, Attractions, Tour Design, Bookings, Contracts, Feedback, and supplier tables from browser snapshot pushes
