...
- `seed.sql` — optional local fixtures after `supabase db reset` (keep minimal)
...
- `wipe-photo-library.sql`, `rls-authenticated.sql`, `fix-product-photos-rls.sql`
...
## Boundaries
- New DDL only via `npm run db:migration:new` → test with `db:push:local` → remote `db:push`; mirror into `schema.sql`.
- Docs: `docs/SUPABASE-SETUP.md` §1 + §5b + §9. Do not put app TypeScript here.
