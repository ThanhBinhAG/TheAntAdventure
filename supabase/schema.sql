...
-- Versioned save prevents a response/request with an older draft state from overwriting newer data.
...
begin
  ...
  if exists (
    select 1 from jsonb_array_elements(p_outline_days) as day
    where coalesce(day ->> 'draft_id', '') <> v_draft_id
  ) then
    raise exception 'Every outline day must belong to the draft' using errcode = '22023';
  end if;
  if exists (
    select 1 from jsonb_array_elements(p_outline_days) as day
    group by day ->> 'day_number' having count(*) > 1
  ) then
    raise exception 'Outline day numbers must be unique' using errcode = '22023';
  end if;
  ...
  select
    (day ->> 'id')::uuid, v_draft_id, (day ->> 'day_number')::smallint,
    nullif(day ->> 'outline_date', '')::date, nullif(day ->> 'location', ''),
    nullif(day ->> 'activities', ''), nullif(day ->> 'hotels', ''),
    coalesce(nullif(day ->> 'sort_order', '')::smallint, (day ->> 'day_number')::smallint)
  from jsonb_array_elements(p_outline_days) as day;
