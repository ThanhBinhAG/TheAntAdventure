...
test('Customer form declares Travel Style hooks before its closed-modal return', () => {
  ...
  const hotelTierMemo = source.indexOf('const selectableHotelTiers = useMemo');
  ...
  assert.ok(hotelTierMemo < closedReturn, 'Hotel Tier hooks must be declared before the closed-modal return');
});
