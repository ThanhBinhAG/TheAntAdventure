...
test('all 91 API routes use a supported HTTP logging contract', () => {
  const root = process.cwd();
  const routes = collectRouteFiles(join(root, 'app/api'));

  assert.equal(routes.length, 91, 'update this acceptance count when an API route is added or removed');
