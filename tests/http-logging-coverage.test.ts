...
test('all 104 API routes use a supported HTTP logging contract', () => {
  const root = process.cwd();
  const routes = collectRouteFiles(join(root, 'app/api'));

  assert.equal(routes.length, 104, 'update this acceptance count when an API route is added or removed');
